...
| Dock and reopen behavior | Green | 冷启动、重复启动、`nexus://open` 和 `nexus://launcher` 默认显示主窗口完整 launcher 首页；Dock 点击只恢复现有主窗口，不主动改写当前路由。 | 多窗口后补 settings / launcher 恢复策略。 |
...
- 默认入口改为 launcher：冷启动、重复启动已有实例、`nexus://open` 和 `nexus://launcher` 都不再直接进入 `/app`；Dock reopen 保留当前主窗口路由。
