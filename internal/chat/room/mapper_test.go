...
func TestSlotMessageMapperUsesAssistantMessageIDForStream(t *testing.T) {
	...
	if events[0].MessageID != "assistant-chat-1" {
		t.Fatalf("Room stream 应使用 assistant message_id，而不是 slot msg_id: %+v", events[0])
	}
	if events[0].Data["message_id"] != "assistant-chat-1" {
		...
		Assistant: &sdkprotocol.AssistantMessage{
			Message: sdkprotocol.ConversationEnvelope{
				Content: []sdkprotocol.ContentBlock{
					sdkprotocol.ToolUseBlock{ID: "tool-chat-1", Name: "WebSearch"},
				},
			},
		},
	}); err != nil {
