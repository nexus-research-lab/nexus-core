...
func TestServiceManagesWorkspaceFiles(t *testing.T) {
	...
	agentValue, err := agentService.CreateAgent(ctx, agent2.CreateRequest{Name: "工作区测试助手"})
	if err != nil {
...
func TestServicePublishesWorkspaceLiveEvents(t *testing.T) {
	...
	agentValue, err := agentService.CreateAgent(ctx, agent2.CreateRequest{Name: "工作区实时助手"})
	if err != nil {
		t.Fatalf("创建 agent 失败: %v", err)
	}
