...
func TestRoomServiceDeletesFinalTopicConversation(t *testing.T) {
	cfg := newRoomTestConfig(t)
	migrateRoomSQLite(t, cfg.DatabaseURL)
	...
	ctx := context.Background()
	agentA := createTestAgent(t, agentService, ctx, "最终话题助手A")
	agentB := createTestAgent(t, agentService, ctx, "最终话题助手B")
	...
	topicAgentASession := seedRoomPrivateSession(
		t,
		files,
	...
	seedRoomConversationLog(t, cfg.WorkspacePath, topicContext.Conversation.ID, topicContext.Room.ID)
	topicAgentADBSessionID := findRoomSessionID(t, *topicContext, agentA.AgentID)
	...
	// 模拟迁移期只剩 topic 的旧数据，验证删除逻辑不再强制保留一个对话。
	if _, err = db.Exec(`DELETE FROM sessions WHERE conversation_id = ?`, mainContext.Conversation.ID); err != nil {
		t.Fatalf("删除主对话 session 失败: %v", err)
	}
	if _, err = db.Exec(`DELETE FROM conversations WHERE id = ?`, mainContext.Conversation.ID); err != nil {
	...
	if len(contexts) != 1 || contexts[0].Conversation.ID != topicContext.Conversation.ID {
		t.Fatalf("测试数据未进入仅剩 topic 状态: %+v", contexts)
	}

	fallbackContext, err := roomService.DeleteConversation(ctx, mainContext.Room.ID, topicContext.Conversation.ID)
	if err != nil {
		t.Fatalf("删除最终 topic 失败: %v", err)
	}
	if fallbackContext != nil {
		t.Fatalf("最终 topic 删除后不应返回回退上下文: %+v", fallbackContext)
	}

	assertPathRemoved(t, paths.RoomConversationDir(topicContext.Conversation.ID))
	...
	assertSQLCount(t, db, `SELECT COUNT(*) FROM sessions WHERE conversation_id = ?`, 0, topicContext.Conversation.ID)
	assertSQLCount(t, db, `SELECT COUNT(*) FROM messages WHERE conversation_id = ?`, 0, topicContext.Conversation.ID)
	assertSQLCount(t, db, `SELECT COUNT(*) FROM rounds WHERE round_id = ?`, 0, topicRoundID)

	contexts, err = roomService.GetRoomContexts(ctx, mainContext.Room.ID)
	if err != nil {
		t.Fatalf("删除后读取 room contexts 失败: %v", err)
	}
	if len(contexts) != 0 {
		t.Fatalf("最终 topic 删除后不应残留上下文: %+v", contexts)
	}
}
