...
type stubConnectorService struct {
	item *connectorsvc.ConnectionSnapshot
}

func (s stubConnectorService) ListActiveConnections(ctx context.Context, ownerUserID string) ([]connectorsvc.ConnectionSnapshot, error) {
	if s.item == nil {
		return nil, nil
	}
	return []connectorsvc.ConnectionSnapshot{*s.item}, nil
}

func (s stubConnectorService) LoadActiveConnection(ctx context.Context, ownerUserID, connectorID string) (*connectorsvc.ConnectionSnapshot, error) {
...
func TestConnectorCallSuccess(t *testing.T) {
	...
	output := callTool(t, stubConnectorService{item: &connectorsvc.ConnectionSnapshot{
		ConnectorID: "github",
		AuthType:    "oauth2",
