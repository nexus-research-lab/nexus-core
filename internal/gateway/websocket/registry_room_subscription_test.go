...
func TestRoomSubscriptionRegistryReplaysDurableEvents(t *testing.T) {
	...
	if err := registry.SubscribeRoom(ctx, senderA, "room-1", "conv-1", nil); err != nil {
		t.Fatalf("首次 subscribe_room 失败: %v", err)
	}
	registry.Broadcast(ctx, "room-1", durableRoomTestEvent(protocol.EventTypeMessage, "conv-1"))
	registry.Broadcast(ctx, "room-1", durableRoomTestEvent(protocol.EventTypeRoundStatus, "conv-1"))

	senderB := newFakeRoomRegistrySender("sender-b")
	lastSeenRoomSeq := int64(1)
	if err := registry.SubscribeRoom(ctx, senderB, "room-1", "conv-1", &lastSeenRoomSeq); err != nil {
...
func TestRoomSubscriptionRegistryRequestsResyncWhenReplayBufferMissed(t *testing.T) {
	...
	lastSeenRoomSeq := int64(1)
	if err := registry.SubscribeRoom(ctx, sender, "room-1", "conv-1", &lastSeenRoomSeq); err != nil {
