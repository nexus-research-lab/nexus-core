...
func TestMiddlewareWritesRequestIDAndAccessLog(t *testing.T) {
	...
	handler := server.requestContextMiddleware(server.accessLogMiddleware(http.HandlerFunc(func(writer http.ResponseWriter, request *http.Request) {
...
func TestRecoverMiddlewareReturnsInternalError(t *testing.T) {
	...
	handler := server.requestContextMiddleware(
		server.accessLogMiddleware(
			server.recoverMiddleware(http.HandlerFunc(func(http.ResponseWriter, *http.Request) {
				panic("boom")
			})),
		),
	)
