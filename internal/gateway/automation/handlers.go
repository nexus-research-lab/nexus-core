...
// New 创建自动化 handlers。
func New(api *gatewayshared.API, automation *automationsvc.Service) *Handlers {
	return &Handlers{
		api:        api,
...
func (h *Handlers) HandleCreateScheduledTask(writer http.ResponseWriter, request *http.Request) {
	...
	item, err := h.automation.CreateTask(request.Context(), automationsvc.CreateJobInput{
		Name:          payload.Name,
...
func (h *Handlers) HandleUpdateScheduledTask(writer http.ResponseWriter, request *http.Request) {
	...
	item, err := h.automation.UpdateTask(request.Context(), chi.URLParam(request, "job_id"), automationsvc.UpdateJobInput{
		Name:          payload.Name,
		Schedule:      payload.Schedule,
		Instruction:   payload.Instruction,
		SessionTarget: payload.SessionTarget,
...
func (h *Handlers) HandleUpdateHeartbeat(writer http.ResponseWriter, request *http.Request) {
	...
	item, err := h.automation.UpdateHeartbeat(request.Context(), chi.URLParam(request, "agent_id"), automationsvc.HeartbeatUpdateInput{
		Enabled:      payload.Enabled,
...
func (h *Handlers) HandleWakeHeartbeat(writer http.ResponseWriter, request *http.Request) {
	...
	item, err := h.automation.WakeHeartbeat(request.Context(), chi.URLParam(request, "agent_id"), automationsvc.HeartbeatWakeRequest{
		Mode: payload.Mode,
		Text: payload.Text,
	})
	if err != nil {
