...
### Changed
- 后端结构继续收口：拆分 `chat/service.go` 为 request / history / round 三段；移除主智能体 ready 流程里提升历史默认 Agent 的兼容分支；ACCESS_TOKEN bearer、memory entry id、session key 校验测试与日志注释等命名收口到当前语义。
