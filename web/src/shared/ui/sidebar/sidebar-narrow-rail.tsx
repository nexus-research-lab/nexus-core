      ...
      <div className="home-glass-panel radius-shell-xl flex h-full w-full flex-col items-center px-2 py-4">
        ...
        <nav className="flex w-full flex-1 flex-col items-center gap-2">
          ...
          <div ref={more_ref} className="relative w-full">
            ...
            {/* More 弹出菜单 — 使用 Portal 避免 overflow-hidden 裁切 */}
            {is_more_open ? createPortal(
              <div
                className="fixed z-[9990] w-48 animate-in fade-in slide-in-from-left-2 duration-150"
                style={(() => {
                  const rect = more_ref.current?.getBoundingClientRect();
                  if (!rect) return { top: 0, left: 0 };
                  return { bottom: window.innerHeight - rect.bottom, left: rect.right + 8 };
                })()}
              >
