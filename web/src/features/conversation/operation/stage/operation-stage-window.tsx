...
export function OperationStageWindow({
  ...
  return (
    <div
      aria-label={title}
      className={cn(
        "operation-stage-window absolute flex min-h-0 flex-col overflow-hidden rounded-[14px] border backdrop-blur-xl max-md:relative max-md:left-auto max-md:right-auto max-md:top-auto max-md:bottom-auto max-md:h-auto max-md:min-h-[180px] max-md:w-full",
        tone === "terminal"
    ...
    >
      ...
      <div className={cn("soft-scrollbar relative min-h-0 flex-1 overflow-auto p-4", minimized && "hidden")}>
        <div
          className={cn(
            "pointer-events-none absolute bottom-3 right-3 flex h-8 w-8 items-center justify-center rounded-[10px] border opacity-30",
            tone === "terminal"
              ? "border-white/10 bg-white/[0.04] text-[rgba(233,241,244,0.72)]"
              : "border-(--divider-subtle-color) bg-white/72 text-(--icon-muted)",
          )}
        >
